...
class AdminRequestOverviewController extends Controller
{
  ...
  public function submitRequestApproval(Request $request)
  {
    ...
    // old locked / unlocked phases:
    $appraisal = Appraisals::find($appraisalID);

    $old_kra = $appraisal->kra_locked;
    $old_pr = $appraisal->pr_locked;
    $old_eval = $appraisal->eval_locked;

    $deadlineType = '';

    if ($old_kra != $kra) {
      $deadlineType .= $kra ? 'kra_' : '';
    }
    ...
    if ($old_eval != $eval) {
      $deadlineType .= $eval ? 'eval_' : '';
    }

    $deadlineType = rtrim($deadlineType, '_');

    Log::debug("Deadline Type:" . $deadlineType);
    ...
    try {
      ...
      if ($appraisal) {
        if (in_array($appraisal->evaluation_type, ["self evaluation", "is evaluation"])) {
